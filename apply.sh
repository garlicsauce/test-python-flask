...
original_author="garlicsauce"
original_name="test_python_flask"
original_urlname="test-python-flask"
original_description="Awesome test_python_flask created by garlicsauce" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test_python_flask "${name}"
...
then
    ...
    echo "then run 'test_python_flask' or 'python -m test_python_flask'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test_python_flask' or 'python -m test_python_flask' or 'poetry run test_python_flask'"
fi
